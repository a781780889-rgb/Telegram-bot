...
const phoneRequestMessage =
  ...
  `مثال: \`+967771234567\`\n\n` +
  `⚠️ سيقبل البوت رمزًا يصل داخل تطبيق تيليجرام فقط. يجب أن يكون الحساب مفتوحًا على جهاز تيليجرام تملكه؛ لن يتم قبول SMS أو المكالمات.`;

const otpRequestMessage = (phone) =>
  `✉️ *تم إرسال رمز التحقق داخل Telegram*\n\n` +
  `تم إرسال رمز التحقق إلى تطبيق تيليجرام على الرقم:\n` +
  `\`${maskPhone(phone)}\`\n\n` +
  `📲 *أين تجد الرمز؟*\n` +
  `افتح تطبيق تيليجرام على جهاز تستخدمه بنفس الرقم، وستجد رسالة جديدة من حساب *Telegram* في قائمة المحادثات. الرمز مكون من 5 أرقام.\n\n` +
  `⚠️ إذا لم تجد الرسالة، تحقق من الأجهزة النشطة وافتح جلسة تيليجرام على جهاز تملكه، ثم أعد المحاولة. لا يقبل البوت رمز SMS أو المكالمة.\n\n` +
