...
// الاشتراك يحدد مدة الصلاحية فقط ولا يقيّد عدد الحسابات.
const PLAN_TYPES = {
  DAYS_30: { key: '30d', label: '30 يوم', days: 30 },
  DAYS_60: { key: '60d', label: '60 يوم', days: 60 },
  DAYS_90: { key: '90d', label: '90 يوم', days: 90 },
  YEAR: { key: '1y', label: 'سنة', days: 365 },
  LIFETIME: { key: 'lifetime', label: 'مفتوح', days: null },
};
...
const getAccountCount = (userId) => Number(getDb().prepare('SELECT COUNT(*) AS count FROM accounts WHERE user_id = ?').get(String(userId))?.count || 0);
...
const getAccess = (userId) => {
  ...
  // لا يوجد حد للحسابات في النظام المبسط؛ انتهاء المدة فقط يلغي الصلاحية.
  return { allowed: true, reason: null, subscription, used, remaining: null };
};

const formatDate = (value) => value
  ? new Intl.DateTimeFormat('ar-SA', { calendar: 'gregory', dateStyle: 'medium', timeZone: 'UTC' }).format(new Date(value))
  : 'غير محدد';
...
const formatSubscription = (subscription, used = getAccountCount(subscription?.user_id)) => {
  ...
  return [
    ...
    `الباقة: ${subscription.plan_label}`,
    `تاريخ التفعيل: ${formatDate(subscription.started_at)}`,
    `تاريخ الانتهاء: ${lifetime ? '♾ مفتوح' : formatDate(subscription.expires_at)}`,
    `الأيام المتبقية: ${daysLeft}`,
    `الحسابات المضافة: ${used}`,
    'إضافة الحسابات: ✅ مسموحة بلا حد',
  ].join('\n');
};
...
const createCodes = ({ planKey, quantity = 1, adminId }) => {
  ...
  const insert = db.prepare(`
    INSERT INTO activation_codes
      (code, plan_key, plan_label, duration_days, max_accounts, status, created_by)
    VALUES (?, ?, ?, ?, ?, 'unused', ?)
  `);
  return db.transaction(() => {
    ...
    for (let i = 0; i < count; i += 1) {
      ...
      insert.run(code, plan.key, plan.label, plan.days, DEFAULT_MAX_ACCOUNTS, String(adminId));
      codes.push(code);
    }
    db.prepare('INSERT INTO admin_actions (admin_id, action, target, metadata) VALUES (?, ?, ?, ?)')
      .run(String(adminId), 'create_activation_codes', null, JSON.stringify({ planKey, quantity: count }));
    return codes;
  })();
};
...
const activateCode = (userId, rawCode) => {
  ...
  return db.transaction(() => {
    ...
    const started = current?.status === 'active' && current.expires_at && new Date(current.expires_at) > new Date()
      ? new Date(current.expires_at)
      : new Date();
    const expires = activation.duration_days === null ? null : futureIso(activation.duration_days, started);
    const inserted = db.prepare(`
      INSERT INTO subscriptions
        (user_id, activation_code_id, plan_key, plan_label, duration_days, max_accounts, started_at, expires_at, status)
      VALUES (?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, ?, 'active')
    `).run(String(userId), activation.id, activation.plan_key, activation.plan_label, activation.duration_days, DEFAULT_MAX_ACCOUNTS, expires);
    const insertedSubscription = db.prepare('SELECT id FROM subscriptions WHERE activation_code_id = ? ORDER BY id DESC LIMIT 1').get(activation.id);
    const subscriptionId = insertedSubscription?.id;
    if (!subscriptionId) return { ok: false, reason: 'storage_error' };

    const consumed = db.prepare(`
      UPDATE activation_codes
      SET status = 'used', used_by = ?, activated_at = CURRENT_TIMESTAMP
      WHERE id = ? AND status = 'unused'
    `).run(String(userId), activation.id);
    if (!consumed.changes) return { ok: false, reason: 'used' };
    ...
    db.prepare(`INSERT INTO subscription_events (user_id, subscription_id, event_type, metadata) VALUES (?, ?, 'activated', ?)`)
      .run(String(userId), subscriptionId, JSON.stringify({ code, previousSubscriptionId: current?.id || null }));
    const subscription = db.prepare('SELECT * FROM subscriptions WHERE id = ?').get(subscriptionId);
    return subscription ? { ok: true, subscription } : { ok: false, reason: 'storage_error' };
  })();
};

const cancelCode = (code, adminId) => getDb().prepare(`
  UPDATE activation_codes SET status = 'cancelled', cancelled_by = ?, cancelled_at = CURRENT_TIMESTAMP
  WHERE code = ? AND status = 'unused'
`).run(String(adminId), normalizeCode(code));
